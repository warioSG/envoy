...
void HttpRouteManagerImpl::addRouteConfigProvider(
    const std::string& name, std::weak_ptr<Router::RouteConfigProvider> route_config_provider) {
  route_config_providers_.insert({name, route_config_provider});
};

Router::RouteConfigProviderSharedPtr
HttpRouteManagerImpl::getRouteConfigProvider(const std::string& name) {
